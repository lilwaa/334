...
class Splitter:
    ...
    def get_all_subsets(self):
        df = pd.read_csv("clean-data-var.csv")
        ...
        self.xtrain = combine_csv_files("var/xtrain", flatten=False)
        self.xtest = combine_csv_files("var/xtest",flatten=False)
        self.ytrain = combine_csv_files("var/ytrain", flatten=True)
        self.ytest = combine_csv_files("var/ytest", flatten=True)
    ...
    def tt_split(self,label, df_x, df_y):
        ...
        xtrain.to_csv(f"var/xtrain/{label}.csv", index=False)
        xtest.to_csv(f"var/xtest/{label}.csv", index=False)
        ytrain.to_csv(f"var/ytrain/{label}.csv", index=False)
        ytest.to_csv(f"var/ytest/{label}.csv", index=False)
